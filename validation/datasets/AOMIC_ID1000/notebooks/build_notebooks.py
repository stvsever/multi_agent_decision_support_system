...
plt.tight_layout(); plt.show()
"""),
        md("## 2. Head-size scaling and lobe thickness map"),
        code("""
fig, ax = plt.subplots(1, 2, figsize=(15, 5))
if "fs_etiv" in morph:
...
"""),
        md("Naturalistic-viewing connectivity varies clearly between individuals; the engine "
           ...
           "for inspection. Morphometry and connectome enter the ontology as distinct domains."),
    ]
    return nb
...
"""),
        md("Every tier is a filtered projection of this one ontology, so the engine always "
           "receives a clean, non-redundant hierarchy; only the set of present leaf values changes."),
    ]
    return nb
